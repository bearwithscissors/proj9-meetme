"""An Agenda is a list-like container of Appt (appointment).
   ...
   Wyatt Reed for CIS 322, U. Oregon
   Original Author: Michael Young, CIS 322, U. Oregon
...
class Appt:
    ...
    def from_string(cls, txt):
        ...
        begin = datetime.datetime.strptime(appt_begin_text, "%H:%M")
        end =   datetime.datetime.strptime(appt_end_text, "%H:%M")
    ...
    def intersect(self, other, desc=""):
        ...
        return Appt(self.begin, begin_time, end_time, desc)
    ...
    def union(self, other, desc=""):
        ...
        return Appt(self.begin, begin, end, desc)
    ...
    def __str__(self):
        ...
        return str(daystr) + str(begstr) + str(endstr) + str(self.desc)
        ...
        for appt in copy.appts:
            ...
            if appt > freeblock:
                if cur_time < freeblock.end:
                    comp.append(Appt(day,cur_time,freeblock.end, desc))
                    cur_time = freeblock.end
                break
            if cur_time < appt.begin:
                # print("Creating free time from", cur_time, "to", appt.begin)
                app.logger.debug("Creating free time from {} to {} on {}".format(cur_time, appt.begin, day))
                comp.append(Appt(day, cur_time, appt.begin, desc))
